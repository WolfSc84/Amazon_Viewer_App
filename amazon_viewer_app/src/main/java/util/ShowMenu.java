...
public class ShowMenu {
    ...
    public static void showMovies() {
        ...
        do {
            ...
            System.out.println();
            response = AmazonUtil.validateUserResponseMenu(0, movies.size());
            if (response == 0) {
                exit = false;
                showMenu();
                break;
    ...
    public static void showChapters(ArrayList<Chapter> chaptersOfSerieSelected) {
        ...
        do {
            ...
            System.out.println("0. Regresar al Menu");
            System.out.println();
            response = AmazonUtil.validateUserResponseMenu(0, chaptersOfSerieSelected.size());
            if (response == 0) {
                exit = false;
            }
    ...
    public static void showBooks() {
        ...
        do {
            ...
            System.out.println("0. Regresar al Menu");
            System.out.println();
            response = AmazonUtil.validateUserResponseMenu(0, books.size());
            if (response == 0) {
                exit = false;
                showMenu();
            }
    ...
    public static void showSeries() {
        ...
        do {
            ...
            for(response = 0; response < series.size(); ++response) {
                System.out.println(response + 1 + ". " + ((Serie)series.get(response)).getTitle() + " Visto: " + ((Serie)series.get(response)).isViewed());
            }

            System.out.println("0. Regresar al Menu");
            System.out.println();
            response = AmazonUtil.validateUserResponseMenu(0, series.size());
            if (response == 0) {
                exit = false;
                showMenu();
            }
            ...
            if (response > 0) {
                showChapters(((Serie)series.get(response - 1)).getChapters());
            }
        } while(exit);
    ...
    public static void showMagazines() {
        ...
        do {
            ...
            for(response = 0; response < magazines.size(); ++response) {
                System.out.println(response + 1 + ". " + ((Magazine)magazines.get(response)).getTitle());
            }

            System.out.println("0. Regresar al Menu");
            System.out.println();
            response = AmazonUtil.validateUserResponseMenu(0, 0);
            if (response == 0) {
                exit = false;
                showMenu();
            }
